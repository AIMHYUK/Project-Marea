...
namespace Marea.Data
{
    ...
    public struct RecipeEntry
    {
        ...
        // 기획 필드명은 RequiredAmount 지만 count 그대로 둔다 (#47). 이름만 바꾸는 건데
        // 그러면 B의 MenuCardSlot·SkewerMinigameController 를 같이 고쳐야 한다.
        // 리네임은 B와 합의하고 따로 한다.
        ...
        ///
        /// 기획이 값을 채울 자리를 여는 것까지가 이 변경이다 — **아직 아무도 안 읽는다.**
        /// 지금은 SkewerMinigameController가 배열 순서에 암묵 의존한다(인스펙터에서 행을
        /// 위아래로 옮기면 조리 순서가 조용히 바뀐다). 그걸 이 값 기준으로 바꾸는 건
        /// B 파일을 고치는 일이라 합의하고 따로 한다. (#47)
        /// </summary>
